...
use Illuminate\Support\Facades\Mail;  //untuk mail
    ...
    public function show(string $id): View 
{
    ...
    $transaksi->email_pembeli = $request->input('email_pembeli');
    ...
    $this->mail($request->email_pembeli, $transaksi->id);
...
//mail controller
public function mail($to, $id)
{
    $transaksi_model = new Transaksi;
    
    // Get the transaction with the details
    $transaksi = $transaksi_model->get_transaksi()->where("transaksi.id", $id)->first();
    ...
    $products = DB::table('detail_transaksi as dt')
                ...
                ->select(
                    'products.title',
                    'dt.jumlah as quantity',
                    'products.price',
                    DB::raw('SUM(dt.jumlah * products.price) as total_harga'),
                    DB::raw('dt.jumlah * products.price as total_price')
                )
                ->groupBy('products.title', 'dt.jumlah', 'products.price') // Group by columns in SELECT
                ->get();

    // Calculate the cumulative total for the transaction
    $total_harga = $products->sum('total_price');
    ...
    // Send the email
    Mail::send('transaksi.mail', $transaksi_, function($message) use ($to, $transaksi, $total_harga) {
        $message->to($to)
            ->subject("Transaksi Details: {$transaksi->email_pembeli} - dengan total tagihan Rp" . number_format($total_harga, 2, ',', '.'));
    });
